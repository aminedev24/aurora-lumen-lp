    ...
    <>
      <header className="relative bg-white dark:bg-gray-800 shadow-lg py-4 px-6 md:px-10 lg:px-16 flex justify-between items-center border-b border-primary-light dark:border-gray-700 transition-colors duration-300">
        <div className="flex items-center">
          <Link to="/" className="flex items-center group">
            ...
            <h1 className="text-2xl font-extrabold text-gray-800 dark:text-dark-text">Aurora Lumen</h1>
          </Link>
        </div>

        <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 space-x-8">
          <Link to="/#features" className="text-gray-700 hover:text-secondary font-medium transition duration-300 transform hover:scale-105 dark:text-dark-text dark:hover:text-secondary">Features</Link>
          <Link to="/#about" className="text-gray-700 hover:text-secondary font-medium transition duration-300 transform hover:scale-105 dark:text-dark-text dark:hover:text-secondary">About</Link>
          <Link to="/#cta" className="text-gray-700 hover:text-secondary font-medium transition duration-300 transform hover:scale-105 dark:text-dark-text dark:hover:text-secondary">Newsletter</Link>
        </nav>
      ...
      {isMenuOpen && (
        <nav className="md:hidden bg-white dark:bg-gray-800 shadow-lg rounded-b-xl mx-4 mt-2 py-2 mobile-nav-menu transition-colors duration-300">
          <Link to="/#features" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md transition duration-300 dark:text-dark-text dark:hover:bg-gray-700">Features</Link>
          <Link to="/#about" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md transition duration-300 dark:text-dark-text dark:hover:bg-gray-700">About</Link>
          <Link to="/#cta" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md transition duration-300 dark:text-dark-text dark:hover:bg-gray-700">Newsletter</Link>
        </nav>
      )}
    </>
  );
